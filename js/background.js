...
function randomBackground(settings, forceReload = false) {
    ...
    if (settings['photoLastCall'] !== "") {
        var storedTime = new Date(settings['photoLastCall']);
    ...
    if (getFreshPhoto || forceReload) {
        ...
        if (settings['photoService'] === 'pixabay') {
            pixabayPhoto(settings, width);
        } else if (settings['photoService'] === 'unsplash') {
            unsplashPhoto(settings);
        } else if (settings['photoService'] === 'googleearth') {
            googleEarthPhoto(settings, googleEarthImageIds);
        } else if (settings['photoService'] === 'reddit') {
            source = new RedditPhoto();
            photo = source.getPhoto();
            // console.log(photo)
            // setBackground(photo);
            // updateSyncSettings('currentPhoto', photo);
        }
    } else {
        setBackground(settings['currentPhoto']);
    }
}
...
function unsplashPhoto(settings) {
    var categories = ['nature'];
    ...
    updateSyncSettings('currentPhoto', photo.url);
}
...
function pixabayPhoto(settings, width) {
    ...
    url += settings['pixabaySearchTerm'] ? 'q=' + encodeURIComponent(settings['pixabaySearchTerm']) : 'editors_choice=true';
    if (settings['pixabaySafeSearch']) url += '&safesearch=true';
